...
class securityService
{
     ...
     * @return string
     */
    public function insertHiddenToken($form)
    {
        $csrfToken = $this->getCSRFToken();

        echo "<!--\n--><input type=\"hidden\"" . ' name="' . $this->xssafe($this->formTokenLabel . $form) . '"' . ' value="' . $this->xssafe($csrfToken) . '"' . ' />';
    }
    ...
    public function validate($form)
    {
        ...
        if (!\in_array($currentUrl, $this->excludeUrl)) {
            if (!empty($this->post)) {
                $isAntiCSRF = $this->validateRequest($form);
                if (!$isAntiCSRF) {
                    // CSRF attack attempt
     ...
     * @return boolean
     */
    public function isValidRequest($form)
    {
        $isValid    = false;
        $currentUrl = $this->getCurrentRequestUrl();
        if (!\in_array($currentUrl, $this->excludeUrl)) {
            if (!empty($this->post)) {
                $isValid = $this->validateRequest($form);
            }
        }
        return $isValid;
    }
    ...
    public function validateRequest($form)
    {
        ...
        if (!empty($this->post[$this->formTokenLabel . $form])) {
            // Let's pull the POST data
            $token = $this->post[$this->formTokenLabel . $form];
        } else {
            return false;
        }
